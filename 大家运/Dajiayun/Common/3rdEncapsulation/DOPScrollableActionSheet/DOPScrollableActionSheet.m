...
- (instancetype)initWithActionArray:(NSArray *)actions {
    ...
    if (self) {
        ...
        for (int i = 0; i < rowCount; i++) {
            if ([_actions[i] isKindOfClass:[NSString class]]) {
                ...
                for (int j = 0; j < items.count; j++) {
                    ...
                    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(Windows_WIDTH/items.count*(j), 50, Windows_WIDTH/items.count, 30)];
                    button.center = CGPointMake(label.center.x, 30);
                    label.text = action.actionName;
                    label.font = [UIFont systemFontOfSize:13.0];
                    label.textColor = UIColorDJYThemecolorsRGB;
        ...
        [cancel setTitleColor:RGBColor(242, 133, 65) forState:UIControlStateNormal];
