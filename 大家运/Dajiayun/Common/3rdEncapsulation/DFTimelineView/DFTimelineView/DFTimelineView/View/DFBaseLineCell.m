...
@interface DFBaseLineCell()<DFLikeCommentToolbarDelegate, DFLikeCommentViewDelegate,MLLinkClickLabelDelegate>
...
@property (strong, nonatomic) MLClickColorLinkLabel *delLabel;
...
-(void) initBaseCell
{
    ...
    if (_delLabel == nil) {
        ...
        _delLabel =[[MLClickColorLinkLabel alloc] initWithFrame:CGRectZero];
        _delLabel.clickDelegate = self;
        _delLabel.font = [UIFont systemFontOfSize:10];
        _delLabel.numberOfLines = 1;
        _delLabel.adjustsFontSizeToFitWidth = NO;
        _delLabel.textInsets = UIEdgeInsetsZero;
        
        _delLabel.dataDetectorTypes = MLDataDetectorTypeAll;
        _delLabel.allowLineBreakInsideLinks = NO;
        _delLabel.linkTextAttributes = nil;
        _delLabel.activeLinkTextAttributes = nil;
        _delLabel.lineHeightMultiple = LikeLabelLineHeight;
        _delLabel.linkTextAttributes = @{NSForegroundColorAttributeName: HighLightTextColor};
        
        NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc]initWithString:@"删除"];
        _delLabel.attributedText = attrStr;
        
        [self.contentView addSubview:_delLabel];
    }
...
-(void)onClickOutsideLink:(long long)uniqueId
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(DelDFTextImageLineItem:Row:)]) {
        [_delegate DelDFTextImageLineItem:self.item.itemId Row:self.item.IndexPathRow];
    }
}
...
-(void)updateWithItem:(DFBaseLineItem *)item
{
    ...
    NSString *memberID = [defaults valueForKey:@"CachesDirectory_MemberInfo_MemberID"];
    self.item = item;
    if (self.item.userId == [memberID integerValue]) {
        _delLabel.hidden = NO;
    }else{
...
-(void) updateLocationLikeComment:(CGFloat)height
{
    ...
    [_delLabel sizeToFit];
    _delLabel.frame = CGRectMake(x+width+10, y, 25, 15);
    ...
    _likeCommentToolbar.frame = CGRectMake(x, y-4, width, height);
...
-(void)onReport
{
    ...
    if (_delegate != nil && [_delegate respondsToSelector:@selector(onReport:)]) {
        [_delegate onReport:self.item.itemId];
    }
}
