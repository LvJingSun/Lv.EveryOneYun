...
#define secondBackColor [UIColor colorWithRed:250.0f/255.0f green:250.0f/255.0f blue:250.0f/255.0f alpha:1.0]
// 选中颜色加深
...
- (instancetype)initWithColumn:(NSInteger)column leftOrRight:(NSInteger)leftOrRight leftRow:(NSInteger)leftRow rightrow:(NSInteger)rightrow threerow:(NSInteger)threerow;{
    self = [super init];
    if (self) {
        _column = column;
...
+ (instancetype)indexPathWithCol:(NSInteger)col leftOrRight:(NSInteger)leftOrRight leftRow:(NSInteger)leftRow rightrow:(NSInteger)rightrow threerow:(NSInteger)threerow{
    JSIndexPath *indexPath = [[self alloc] initWithColumn:col leftOrRight:leftOrRight leftRow:leftRow rightrow:rightrow threerow:threerow ];
    return indexPath;
}
@end
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _threeTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.size.width+_rightTableView.frame.origin.x, self.frame.origin.y + self.frame.size.height, 0, 0) style:UITableViewStyleGrouped];
        _threeTableView.rowHeight = 38;
        _threeTableView.separatorColor = [UIColor colorWithRed:220.f/255.0f green:220.f/255.0f blue:220.f/255.0f alpha:1.0];
        ...
        _threeTableView.autoresizesSubviews = NO;
    ...
    if (displayByCollectionView) {
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_currentSelectedMenudIndex!=-1) {
                // 需要隐藏tableview
                [self animateLeftTableView:_leftTableView rightTableView:_rightTableView threeTableView:_threeTableView show:NO complete:^{
        ...
        UITableView *threeTableView = nil;
        
        if (haveRightTableView) {
            threeTableView = _threeTableView;
            // 修改左右tableview显示比例
        }
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView leftTableView:_leftTableView rightTableView:_rightTableView threeTableView:_threeTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
                _currentSelectedMenudIndex = tapIndex;
                _show = NO;
            }];
            ...
            if ([_dataSource respondsToSelector:@selector(currentRightSelectedRow:)]) {
                
                _rightSelectedRow = [_dataSource currentRightSelectedRow:_currentSelectedMenudIndex];
...
//            [_threeTableView reloadData];
            ...
            if (_threeTableView) {
                ...
                _threeTableView.frame = CGRectMake(_origin.x+_leftTableView.frame.size.width+_rightTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, 0);
            }
            ...
            if (_currentSelectedMenudIndex!=-1) {
                ...
                [self animateCollectionView:_collectionView show:NO complete:^{
                    ...
                    [self animateIdicator:_indicators[tapIndex] background:_backGroundView leftTableView:_leftTableView rightTableView:_rightTableView threeTableView:_threeTableView title:_titles[tapIndex] forward:YES complecte:^{
                        _show = YES;
                    }];
                }];
 ...
 *动画显示下拉菜单
 */
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView threeTableView:(UITableView *)threeTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        CGFloat threeTableViewHeight = 0;
        ...
        if (rightTableView) {
            ...
            rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, 0);
        ...
        if (threeTableView) {
            ...
            threeTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width+rightTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, 0);
            
            [self.superview addSubview:threeTableView];
            
            threeTableViewHeight = ([threeTableView numberOfRowsInSection:0] > 5) ? (5 * threeTableView.rowHeight) : ([threeTableView numberOfRowsInSection:0] * threeTableView.rowHeight);
        }
        CGFloat threeViewHeight = MAX(leftTableViewHeight, rightTableViewHeight);
        CGFloat tableViewHeight = MAX(threeViewHeight, threeTableViewHeight);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (threeTableView) {
                threeTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width+rightTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, self.frame.size.width*ratio, tableViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (threeTableView) {
                [threeTableView removeFromSuperview];
            }
        }];
    }
    complete();
...
- (void)animateIdicator:(CAShapeLayer *)indicator background:(UIView *)background leftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView threeTableView:(UITableView *)threeTableView title:(CATextLayer *)title forward:(BOOL)forward complecte:(void(^)())complete{
    
    [self animateIndicator:indicator Forward:forward complete:^{
        [self animateTitle:title show:forward complete:^{
            [self animateBackGroundView:background show:forward complete:^{
                [self animateLeftTableView:leftTableView rightTableView:rightTableView threeTableView:threeTableView show:forward complete:^{
                }];
            }];
        }];
    }];
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        titleLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow rightrow:_rightSelectedRow threerow:indexPath.row]];
    ...
    if (leftOrRight == 2) {
        ...
        titleLabel.frame = CGRectMake(marginX, 0, textSize.width, cell.frame.size.height);
        //右边tableview
        cell.backgroundColor = secondBackColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if (_threeTableView==tableView) {
        leftOrRight = 2;
    } else  {
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        if ((leftOrRight<=1 && !haveRightTableView) || leftOrRight==2 ||indexPath.row==0) {
            [self confiMenuWithSelectRow:indexPath.row leftOrRight:leftOrRight];
        }
        
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow rightrow:_rightSelectedRow threerow:indexPath.row]];
...
//            if (!_hadSelected) {
                ...
                _hadSelected = YES;
                [_rightTableView reloadData];
                NSIndexPath *selectedIndexPath = [NSIndexPath indexPathForRow:_rightSelectedRow inSection:0];
                [_rightTableView selectRowAtIndexPath:selectedIndexPath animated:NO scrollPosition:UITableViewScrollPositionNone];
            }
//        }
        [_threeTableView reloadData];
        }
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    if (leftOrRight>0 && row==0) {
        title.string = [self.dataSource menuFirstTitle:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow rightrow:_rightSelectedRow threerow:row]];
    }else{
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow rightrow:_rightSelectedRow threerow:row]];
    }
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView leftTableView:_leftTableView rightTableView:_rightTableView threeTableView:_threeTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        cell.textLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 rightrow:-1 threerow:indexPath.row]];
    } else {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 rightrow:-1 threerow:indexPath.row ]];
    } else {
        //TODO: delegate is nil
    }
}
...
- (void)confiMenuWithSelectRow:(NSInteger)row{
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 rightrow:-1 threerow:row]];
