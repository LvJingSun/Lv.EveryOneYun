...
+ (NSString *)stringDaysAgoAgainstMidnightForNormal:(BOOL)flag withDate:(NSDate *)date {
    ...
    switch (daysAgo) {
        ...
        default:
        {
           NSString *datestring =[GJCFDateUitil stringFromDate:date withFormat:kNSDateHelperFormatSQLNO_Date];
           text =  GJCFDateToStringByFormat(GJCFDateFromStringByFormat(datestring,kNSDateHelperFormatSQLNO_Date), kNSDateHelperFormatSQLDate);
...
+ (NSString *)stringDaysAgoForNormal:(NSDate *)date
{
    if (GJCFCheckObjectNull(date)) {
        return nil;
    }
    return [GJCFDateUitil stringDaysAgoAgainstMidnightForNormal:YES withDate:date];
}
...
+ (NSString *)dateToStringByInteger:(NSInteger)datemonth
{
    NSArray *constellations = @[
                                @"一", @"二", @"三", @"四", @"五", @"六",
                                @"七", @"八", @"九", @"十", @"十一", @"十二"
                                ];
    return constellations[datemonth-1];
