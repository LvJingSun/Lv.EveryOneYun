...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (model.isSender) {
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[CachesDirectory getValueByKey:CachesDirectory_MemberInfo_PhotoMid]] placeholderImage:model.avatarImage];
        _nameLabel.text = [CachesDirectory getValueByKey:CachesDirectory_MemberInfo_NickName];
        ...
        member = [ContactsDao queryDataMember:[username intValue]];
        ...
        if (head) {
            self.avatarView.image = head;
        }else{
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:member.photoMid] placeholderImage:model.avatarImage];
        }
        //聊天类中不用频繁请求
        if (![ContactsDao queryDataISMember:[username intValue]]) {
            [self getmemberInfoFrmoID:model Ids:username];
        }
    }
...
//*根据id获取会员信息*********************************************
- (void)getmemberInfoFrmoID:(id<IMessageModel>)model Ids:(NSString *)Ids {
    ...
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/GetMemberInfoByIds" params:params networkBlock:^{
        ...
        if (success) {
            ...
            if ([memberList isKindOfClass:[NSArray class]]&&memberList.count!=0) {
                ...
                _nameLabel.text = Member.nickName;
                [self.avatarView sd_setImageWithURL:[NSURL URLWithString:Member.photoMid] completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
