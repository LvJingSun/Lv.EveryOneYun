...
- (void)commonInit
{
    if ([UIDevice currentDevice].systemVersion.floatValue >= 7.0) {
        [[UINavigationBar appearance] setBarTintColor:UIColorDJYThemecolorsRGB];
...
-(void)didAutoLoginWithInfo:(NSDictionary *)loginInfo error:(EMError *)error
{
    ...
    if (error) {
        ...
        [self DJYlogin:keyUserName password:keyPassWord];
        //获取群组列表
...
- (void)DJYlogin:(NSString *)username
    ...
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/MemberLogin" params:params networkBlock:^{
        ...
        [SVProgressHUD showErrorWithStatus:@"大家运系统登录失败"];
    }];
