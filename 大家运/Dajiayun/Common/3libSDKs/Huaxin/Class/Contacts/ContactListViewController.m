...
- (NSString*)showName
{
    Contact *member =[ContactsDao queryGCDDataMember:[self.username intValue]];
...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setCellForRowAtIndexPathCompletion:^UITableViewCell *(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            Contact *member =[ContactsDao queryDataMember:[buddy.username intValue]];
            cell.textLabel.text = member.nickName;
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:member.photoMid]];
...
//        cell.model = nil;
        if (indexPath.row == 0) {
            ...
            cell.avatarView.image = [UIImage imageNamed:@"iconfont-xinFri"];
//            cell.titleLabel.text = NSLocalizedString(@"title.chatroomlist",@"chatroom list");
            cell.titleLabel.text = @"新朋友";
        }
        ...
        cell.delegate = self;
        [cell.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:model.avatarURLPath]];
...
//        }
            ...
            NewFriendsAdressListViewController *VC = [[NewFriendsAdressListViewController alloc] init];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self DeleteFriendsFrmoID:model.buddy.username];
        if (!error) {
...
//            model.nickname = [[UserProfileManager sharedInstance] getNickNameWithUsername:buddy.username];
            ...
            model.avatarURLPath = member.photoMid;

            NSString *firstLetter = member.nickName.length==0?@"#": [EaseChineseToPinyin pinyinFromChineseString:[[UserProfileManager sharedInstance] getNickNameWithUsername:member.nickName]];
...
- (void)reloadDataSource
{
    ...
    if (loginUsername && loginUsername.length > 0) {
        EMBuddy *loginBuddy = [EMBuddy buddyWithUsername:loginUsername];
        [self.contactsSource addObject:loginBuddy];
        [IDS addObject:loginBuddy.username];

    }

    [self getmemberInfoFrmoID:[IDS componentsJoinedByString:@","]];
}
    ...
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/GetMemberInfoByIds" params:params networkBlock:^{
        ...
        if (success) {
            ...
            if ([memberList isKindOfClass:[NSArray class]]&&memberList.count!=0) {
                [memberList enumerateObjectsUsingBlock:^(NSDictionary *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    Contact *Member = [Contact objectWithKeyValues:obj];
...
//*根据id删除好友*********************************************
- (void)DeleteFriendsFrmoID:(NSString *)Ids {
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                            [CachesDirectory getValueByKey:CachesDirectory_MemberInfo_MemberID],@"memberId",
                            [CachesDirectory getServerKey],@"key",
                            Ids,@"toMemberId",
                            nil];
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/DeleteFriends" params:params networkBlock:^{
    } successBlock:^(AFCustomClient *request, NSJSONSerialization *responseObject) {
    } failedBlock:^(AFCustomClient *request, NSError *error) {
    }];
}
...
//- (void)_sortDataNameArrayName:(NSArray *)contactsSource
//{
//    [self.datanamearray removeAllObjects];
//    [self.sectionTitles removeAllObjects];
//    
//    //建立索引的核心, 返回27，是a－z和＃
//    UILocalizedIndexedCollation *indexCollation = [UILocalizedIndexedCollation currentCollation];
...
//    
//    //按首字母分组
//    for (NSDictionary *buddy in contactsSource) {
//        NSString *username = [buddy objectForKey:@"nickName"];
//        if (buddy) {
...
//            NSString *firstLetter1 = [EaseChineseToPinyin pinyinFromChineseString:[obj1 objectForKey:@"nickName"]];
//            firstLetter1 = [[firstLetter1 substringToIndex:1] uppercaseString];
//            
//            NSString *firstLetter2 = [EaseChineseToPinyin pinyinFromChineseString:[obj2 objectForKey:@"nickName"]];
//            firstLetter2 = [[firstLetter2 substringToIndex:1] uppercaseString];
//            
//            return [firstLetter1 caseInsensitiveCompare:firstLetter2];
//        }];
...
//        }
//    }
//    [self.datanamearray addObjectsFromArray:sortedArray];
//    [self.tableView reloadData];
//}
