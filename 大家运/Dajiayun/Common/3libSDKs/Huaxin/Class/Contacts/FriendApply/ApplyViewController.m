    ...
    if(self.dataSource.count > indexPath.row)
    {
        ...
        if (entity) {
            ...
            else if (applyStyle == ApplyStyleJoinGroup)
            {
                ...
                Contact *member = [ContactsDao queryDataMember:[entity.applicantUsername intValue]];
                cell.titleLabel.text = member.nickName;
                UIImage *head = [self GetCachesDirectoryImagePath:[NSString stringWithFormat:@"%@%@",DJY_headimage_data,member.photoMid]];
                if (head) {
                    cell.headerImageView.image = head;
                }
                //*根据id获取会员信息*********************************************
                    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                                            entity.applicantUsername, @"Ids",
                                            nil];
                        ...
                        if (success) {
                            ...
                            if ([memberList isKindOfClass:[NSArray class]]&&memberList.count!=0) {
                               ...
                               Contact* Member = [Contact objectWithKeyValues:MemberInfo];
                                cell.titleLabel.text = Member.nickName;
                                [cell.headerImageView sd_setImageWithURL:[NSURL URLWithString:Member.photoMid]];
...
- (void)applyCellAddFriendAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        else */if (applyStyle == ApplyStyleJoinGroup)
        {
            ...
            //接受好友请求
            [self requestAddFriends2:entity.applicantUsername];
        }
...
//处理添加好友的申请
- (void)requestAddFriends2:(NSString *)ids{
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                            [CachesDirectory getValueByKey:CachesDirectory_MemberInfo_MemberID],@"memberId",
                            [CachesDirectory getServerKey],@"key",
                            ids, @"toMemberId",
                            nil];
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/AddFriends2" params:params networkBlock:^{
    } successBlock:^(AFCustomClient *request, NSJSONSerialization *responseObject) {
    } failedBlock:^(AFCustomClient *request, NSError *error) {
    }];
}
