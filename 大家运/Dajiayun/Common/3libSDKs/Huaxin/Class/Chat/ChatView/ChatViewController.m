...
@interface ChatViewController ()<UIAlertViewDelegate, EaseMessageViewControllerDelegate, EaseMessageViewControllerDataSource,TTTAttributedLabelDelegate,UIActionSheetDelegate>
{
...
- (void)viewDidLoad {
    ...
    member = [ContactsDao queryDataMember:[self.title intValue]];
    [self getmemberInfoFrmoID:self.title];
...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.conversationType == eConversationTypeChat) {
        ...
        [clearButton setImage:[UIImage imageNamed:@"iconfont-dingdanxiangqinggerenxinxi"] forState:UIControlStateNormal];
...
- (void)messageViewControllerWithString:(NSString *)string {

    NSMutableString *mutstr = [[NSMutableString alloc] init];
    
    NSMutableArray *mutarr = [NSMutableArray array];
    
    mutstr.string = string;
    
    while (([mutstr rangeOfString:@"(?i)(((http|https|ftp):(//|\\\\)|((w)+[.]))[^\u4e00-\u9fa5]+)|(([a-zA-Z])[^\u4e00-\u9fa5]+(.com|.cn|.cc))" options:NSRegularExpressionSearch]).location != NSNotFound) {
        
        [mutarr addObject:[mutstr substringWithRange:[mutstr rangeOfString:@"(?i)(((http|https|ftp):(//|\\\\)|((w)+[.]))[^\u4e00-\u9fa5]+)|(([a-zA-Z])[^\u4e00-\u9fa5]+(.com|.cn|.cc))" options:NSRegularExpressionSearch]]];
        
        [mutstr deleteCharactersInRange:[mutstr rangeOfString:@"(?i)(((http|https|ftp):(//|\\\\)|((w)+[.]))[^\u4e00-\u9fa5]+)|(([a-zA-Z])[^\u4e00-\u9fa5]+(.com|.cn|.cc))" options:NSRegularExpressionSearch]];
    }
    
    if (mutarr.count > 0) {
        
        UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"前往" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:mutarr[0] otherButtonTitles:nil, nil];
        
        for (int i = 1; i < mutarr.count; i ++) {
            
            [sheet addButtonWithTitle:mutarr[i]];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    NSString *urlstr = [actionSheet buttonTitleAtIndex:buttonIndex];
    
    if ([urlstr isEqualToString:@"取消"]) {
...
//        [alertView show];
        UserProfileViewController *userprofile = [[UserProfileViewController alloc] initWithUsername:[NSString stringWithFormat:@"%d",Member.memberId]];
        [self.navigationController pushViewController:userprofile animated:YES];
    }
}
