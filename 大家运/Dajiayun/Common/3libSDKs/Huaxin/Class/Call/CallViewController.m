...
- (void)_setupSubviews
{
    ...
    member = [ContactsDao queryDataMember:[_chatter intValue]];
    ...
    [_headerImageView sd_setImageWithURL:[NSURL URLWithString:member.photoMid]];
...
- (void)_initializeCamera
{
    ...
    if(!error){
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"call.error", @"Error") message:error.localizedFailureReason delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
        [alertView show];
    }
                           ...
                           error:(EMError *)error
{
    ...
    if(error){
        ...
        UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"call.error", @"Error") message:error.description delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
...
+ (BOOL)canVideo
{
    if([[[UIDevice currentDevice] systemVersion] compare:@"7.0"] != NSOrderedAscending){
        if(([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] == AVAuthorizationStatusDenied)){
            ...
            return YES;
        }else if (([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] == AVAuthorizationStatusRestricted)){           UIAlertView * alt = [[UIAlertView alloc] initWithTitle:@"您可能在家长控制环境下无法使用" message:nil delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"ok", @"OK"), nil];
            [alt show];
            return NO;
        }
    }
