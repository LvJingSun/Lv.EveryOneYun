...
@interface MyReleaseProViewController ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate>
{
    NSInteger pageIndex;
...
@implementation MyReleaseProViewController
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if ((int)buttonIndex == 0) {
        ...
        NSString *infoID = [cheInfo objectForKey:@"cheInfoId"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    NSDictionary *cheInfo = self.dataArr[indexPath.row];
    cell.fromAddress.text = [cheInfo objectForKey:@"fromAddress"];
    cell.toAddress.text = [cheInfo objectForKey:@"toAddress"];
    cell.createDate.text = [cheInfo objectForKey:@"createDate"];
    cell.faCheShiJian.text = [cheInfo objectForKey:@"faCheShiJian"];
    cell.statusName.text = [cheInfo objectForKey:@"statusName"];
    cell.statusName.text = [NSString stringWithFormat:@"%@...",[cheInfo objectForKey:@"statusName"]];
    NSString *spcType = [cheInfo objectForKey:@"spcType"];
    cell.cheWeiShu.text = [NSString stringWithFormat:@"空闲车位:%@个(%@)",[cheInfo objectForKey:@"cheWeiShu"],spcType];
//    if ([spcType isEqualToString:@"SPCSml"]) {
//        cell.cheWeiShu.text = [NSString stringWithFormat:@"空闲车位:%@(小)",[cheInfo objectForKey:@"cheWeiShu"]];
//    }else if ([spcType isEqualToString:@"SPCMid"]) {
//        cell.cheWeiShu.text = [NSString stringWithFormat:@"空闲车位:%@(中)",[cheInfo objectForKey:@"cheWeiShu"]];
//    }else if ([spcType isEqualToString:@"SPCBig"]) {
//        cell.cheWeiShu.text = [NSString stringWithFormat:@"空闲车位:%@(大)",[cheInfo objectForKey:@"cheWeiShu"]];
//    }else{
//        cell.cheWeiShu.text = [NSString stringWithFormat:@"空闲车位:%@",[cheInfo objectForKey:@"cheWeiShu"]];
//    }
    ...
    NSString *tuJingChengShi = [cheInfo objectForKey:@"tuJingChengShi"];
...
#pragma mark - 商品车列表
- (void)requestMySPCInfoList{
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                            [CachesDirectory getValueByKey:CachesDirectory_MemberInfo_MemberID],@"memberId",
                            [CachesDirectory getServerKey],@"key",
                            nil];
    [AFCustomClient requestSameWithHttpMethod:HttpMethodPost URLStr:@"MemberInfoWebService.asmx/MySPCInfoList_2" params:params networkBlock:^{
        [self.view makeToast:@"没有网络!"];
    } successBlock:^(AFCustomClient *request, NSJSONSerialization *responseObject) {
        BOOL success = [[responseObject valueForKey:@"Status"] boolValue];
        if (success) {
            self.dataArr = [responseObject valueForKey:@"spcList"];
